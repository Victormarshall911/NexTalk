...
export default function ChatScreen() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (receiverId) {
        const { data } = await supabase.from('profiles').select('full_name, email').eq('id', receiverId).single();
        if (data) setReceiverName(data.full_name || data.email || 'Unknown User');
      }
      setLoading(false);
    };
    fetchData(); 
  ...
  // 2. Real-time Subscription AND Mark as Read
  useEffect(() => {
    if (!currentUser || !receiverId) return;

    // A. Function to Mark Messages as Read
    const markAsRead = async () => {
      await supabase
        .from('messages')
        .update({ is_read: true })
        .eq('user_id', receiverId)       // Sent by THEM
        .eq('receiver_id', currentUser._id) // Received by ME
        .eq('is_read', false);           // Only unread ones
    };

    // Call it immediately upon entering
    markAsRead();
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, (payload) => {
          ...
          // If the message belongs to this conversation
          if ((newMsg.user_id === receiverId && newMsg.receiver_id === currentUser._id) || 
              (newMsg.user_id === currentUser._id && newMsg.receiver_id === receiverId)) {
            ...
            // If WE received it while looking at the screen, mark it read immediately
            if (newMsg.user_id === receiverId) {
              markAsRead(); 
            }
          }
      })
  ...
  const onSend = useCallback(async (newMessages: IMessage[] = []) => {
    ...
    if (error) { console.error("Error sending message to DB:", error); return; }
    ...
    try {
      ...
      if (receiverProfile?.push_token) {
        await sendPushNotification(receiverProfile.push_token, currentUser.name, text);
      }
    } catch (pushError) { console.log("Notification check failed:", pushError); }

  }, [currentUser, receiverId]);

  // ... (Rest of your UI/Render functions remain exactly the same) ...
  const renderBubble = (props: any) => (
    <Bubble {...props} wrapperStyle={{ right: { backgroundColor: colors.tint }, left: { backgroundColor: colors.inputBackground } }} textStyle={{ right: { color: '#fff' }, left: { color: colors.text } }} />
  );

  const renderInputToolbar = (props: any) => (
    <InputToolbar {...props} containerStyle={{ backgroundColor: colors.card, borderTopColor: colors.border, padding: 5 }} />
  );

  const renderComposer = (props: any) => (
    <Composer {...props} textInputStyle={{ color: colors.text, backgroundColor: colors.inputBackground, borderRadius: 20, paddingHorizontal: 12, paddingTop: 8.5 }} />
  );

  const renderSend = (props: any) => (
    <Send {...props}>
      <View style={{ marginBottom: 5, marginRight: 5 }}><Ionicons name="send" size={32} color={colors.tint} /></View>
    </Send>
  );
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15, paddingVertical: 10, borderBottomWidth: 1, height: 60 },
